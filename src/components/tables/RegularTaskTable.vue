  ...
  <div class="flex w-full items-center justify-center mt-10">
    <table class="min-w-full divide-y divide-gray-200">
      ...
      <tbody class="bg-white divide-y divide-gray-200">
        <tr v-for="(item, index) in data.regularTasks" :key="index">
          ...
          <div class="flex items-center justify-center space-x-5">
            <router-link :to="{ name: 'edit-regular-task', params: { id: item.id } }">
...
const deleteItem = (nameID) => {
  axios.delete(`http://localhost:8000/api/regular-task/${nameID}`).then(alert('Deleted'))
}
</script>
