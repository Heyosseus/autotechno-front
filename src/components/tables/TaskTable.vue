    ...
    <table class="w-form divide-y divide-gray-200">
      <thead>
        <tr>
          ...
          >
            ამოცანები პროგრამებისა <br />და პროექტებისთვის
          </th>
          <th
            scope="col"
            class="px-6 py-3 bg-gray-50 text-left text-xl font-medium text-gray-500 uppercase tracking-wider"
      ...
      <tbody class="bg-white divide-y divide-gray-200">
        <tr v-for="(item, index) in data.tasks" :key="index">
          <td class="px-6 py-4 whitespace-nowrap border border-gray-300">{{ item.tasks }}</td>
          ...
          <div class="flex items-center justify-center space-x-5">
            <router-link :to="{ name: 'edit-task', params: { id: item.id } }">
...
const deleteItem = (nameID) => {
  axios.delete(`http://localhost:8000/api/task/${nameID}`).then(alert('Deleted'))
}
</script>
