        ...
        <div class="bg-blue-950 w-[800px] px-20 sm:py-8 rounded-xl" ref="modalRef">
          <div class="modal-content">
            ...
            <form @submit.prevent="AddItems" class="mt-6">
              ...
              <Field name="planned_time" type="text" required v-model="planned_time"></Field>
...
axios.get('http://localhost:8000/api/result').then((response) => {
  ...
  task.value = data.value.trainings.filter((item) => item.id == nameID)[0].training_tasks
  task_product.value = data.value.trainings.filter((item) => item.id == nameID)[0].task_product
  planned_time.value = data.value.trainings.filter((item) => item.id == nameID)[0].planned_time
  factual_time.value = data.value.trainings.filter((item) => item.id == nameID)[0].factual_time
  completion_date.value = data.value.trainings.filter(
    (item) => item.id == nameID
  )[0].completion_date
})
