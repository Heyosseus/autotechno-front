        ...
        <div class="bg-blue-950 w-[800px] px-20 sm:py-8 rounded-xl" ref="modalRef">
          <div class="modal-content">
            ...
            <form @submit.prevent="AddItems" class="mt-6">
              <label for=""> ამოცანები პროგრამებისა და პროექტებისთვის</label>
              <Field name="name" type="text" rules="" v-model="task"></Field>
              <ErrorMessage name="name" class="text-red-500 text-xs" />
              <label for=""> ამოცანის პროდუქტი</label>
              <Field name="dimension" type="text" v-model="task_product"></Field>
              <ErrorMessage name="dimension" />
              <label for=""> დაგეგმილი დრო</label>
              <Field name="quantity" type="text" v-model="planned_time"></Field>
              <label for="">ფაქტიური დრო</label>
              <Field name="quota" type="text" v-model="factual_time"></Field>
              <label for=""> შესრულების თარიღი</label>
              <Field name="note" type="date" v-model="completion_date"></Field>
...
axios.get('http://localhost:8000/api/result').then((response) => {
  ...
  task.value = data.value.tasks.filter((item) => item.id == nameID)[0].tasks
  task_product.value = data.value.tasks.filter((item) => item.id == nameID)[0].task_product
  planned_time.value = data.value.tasks.filter((item) => item.id == nameID)[0].planned_time
  factual_time.value = data.value.tasks.filter((item) => item.id == nameID)[0].factual_time
  completion_date.value = data.value.tasks.filter((item) => item.id == nameID)[0].completion_date
})

const AddItems = () => {
  axios
    .put(`http://localhost:8000/api/task/${nameID}}`, {
      tasks: task.value,
